...
async function main() {
	...
	const roleMap = new Map(
		roles.map((role) => [
			String(role._id),
			role.type as keyof typeof USER_IDENTITY_PREFIXES | undefined,
		]),
	);

	const roleTypes = Object.keys(USER_IDENTITY_PREFIXES) as Array<
		keyof typeof USER_IDENTITY_PREFIXES
	>;
