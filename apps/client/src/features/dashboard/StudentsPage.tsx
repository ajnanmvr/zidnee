...
export const StudentsPage = () => {
	...
	const columns = useMemo(
		() => buildStudentColumns(getStudentStatusColor, mentorNameById, {
			...
			groupLabelByBatchId: (batchesQuery.data?.batches ?? []).reduce<Record<string, string>>((acc, b) => {
		...
		[batchesQuery.data?.batches, mentorNameById, canUpdateStudent],
	);
	...
	const closeAddToGroupModal = () => { setAddToGroupModalOpen(false); setSelectedStudent(null); setSelectedGroupId(""); setGroupSearch(""); };
		...
		<div className="space-y-3">
			...
			<div className="flex gap-1 overflow-x-auto rounded-xl border border-gray-200 bg-gray-50 p-1">
				{COURSE_TABS.map((tab) => {
					...
					return (
						...
						>
							{tab.label}
							{tab.count > 0 ? <span className={`ml-1.5 rounded-full px-1.5 py-0.5 text-[11px] font-bold ${isActive ? "bg-teal-100 text-teal-700" : "bg-gray-200 text-gray-600"}`}>{tab.count}</span> : null}
						</button>
					);
				})}
				<div className="ml-auto flex items-center gap-1 px-1">
					...
					<Link to="/students/dropped" className="rounded-lg border border-gray-200 px-3 py-1.5 text-xs font-semibold text-gray-500 hover:border-red-300 hover:text-red-600">Dropped</Link>
				</div>
			</div>
			...
			<div className="flex flex-wrap items-center gap-2 rounded-xl border border-gray-200 bg-white px-4 py-3">
				<input
					...
					className="w-52 rounded-lg border border-gray-200 px-3 py-1.5 text-sm outline-none focus:border-teal-500"
				/>
				<button type="button" onClick={() => setShowProcessStudents((c) => !c)} className={`rounded-lg px-3 py-1.5 text-sm font-semibold transition ${showProcessStudents ? "bg-amber-500 text-white" : "border border-gray-200 bg-white text-gray-600 hover:border-amber-300 hover:text-amber-700"}`}>
					{showProcessStudents ? "Incl. in process" : "Excl. in process"}
				</button>
				<div className="flex items-center gap-1">
					<button type="button" onClick={() => setLoadAllRequested(false)} className={`rounded-lg px-3 py-1.5 text-sm font-semibold transition ${activeScope === "mine" ? "bg-teal-600 text-white" : "border border-gray-200 bg-white text-gray-600 hover:border-teal-400 hover:text-teal-700"}`}>Mine</button>
					<button type="button" onClick={() => setLoadAllRequested(true)} disabled={!canReadAllStudents} className={`rounded-lg px-3 py-1.5 text-sm font-semibold transition ${activeScope === "all" ? "bg-teal-600 text-white" : "border border-gray-200 bg-white text-gray-600 hover:border-teal-400 hover:text-teal-700"} disabled:opacity-40`}>All</button>
				</div>
				<div className="ml-auto flex items-center gap-2">
					<select value={sortBy} onChange={(e) => setQueryParam("sortBy", e.target.value)} className="rounded-lg border border-gray-200 px-2.5 py-1.5 text-sm outline-none">
						...
						<option value="admittedAt">Admitted</option>
			...
			{studentsQuery.isLoading ? (
				<div className="flex justify-center py-10"><div className="h-6 w-6 animate-spin rounded-full border-2 border-teal-600 border-t-transparent" /></div>
			) : (
				<div className="rounded-xl border border-gray-200 bg-white overflow-hidden">
					<DataTable data={filteredStudents} columns={columns} enableGlobalFilter={false} enableTableSorting={false} />
					{filteredStudents.length === 0 ? (
						<p className="py-10 text-center text-sm text-gray-400">No active students match the current filter.</p>
					) : null}
			...
			<Modal
				...
				footer={
					<>
						...
						<button type="button" onClick={() => void submitAddToGroup()} disabled={!selectedGroupId || updateStudentMutation.isPending} className="rounded-xl bg-emerald-600 px-4 py-2 text-sm font-semibold text-white transition hover:bg-emerald-700 disabled:opacity-60">
