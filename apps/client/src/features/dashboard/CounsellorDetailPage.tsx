...
export const CounsellorDetailPage = () => {
  const { id } = useParams();
  ...
  if (!user) return <div className="p-6">Counsellor not found</div>;

  return (
    <Panel title={user.name} description={`Counsellor ${user.zids?.counsellor ?? user.counsellorId ?? user.username}`}>
      <div className="grid gap-4">
        <div>
          <p className="text-sm text-gray-600">ID</p>
          <p className="text-lg font-semibold">{user.zids?.counsellor ?? user.counsellorId ?? user.username}</p>
        </div>
        <div>
