	...
	return (
		<div className="grid gap-6">
								...
								<tr key={user.id} className="border-t border-border align-top">
									...
									<td className="px-4 py-3">
										<div className="flex flex-wrap gap-2">
											{user.roles.map((role) => (
												<span key={role.id} className="rounded-full border border-brand/20 bg-brand-soft px-3 py-1 text-xs font-semibold text-brand">
													{role.name}
												</span>
											))}
										</div>
									...
									<td className="px-4 py-3">
										<span className={user.isActive ? "rounded-full bg-accent-soft px-3 py-1 text-xs font-semibold text-ink" : "rounded-full bg-danger-soft px-3 py-1 text-xs font-semibold text-ink"}>
											{user.isActive ? "Active" : "Inactive"}
										</span>
									</td>
									<td className="px-4 py-3">
										<div className="flex flex-wrap gap-2">
											<button type="button" className="inline-flex items-center gap-1 rounded-full border border-border px-3 py-1 text-xs font-semibold text-ink transition hover:border-brand/30" onClick={() => navigate(`/users/${user.id}/edit`)}>
												<HiPencilSquare className="h-3.5 w-3.5" aria-hidden="true" />
												Edit
											</button>
											<button type="button" className="inline-flex items-center gap-1 rounded-full border border-sky/30 bg-sky-soft px-3 py-1 text-xs font-semibold text-ink transition hover:brightness-105" onClick={() => { setPasswordUserId(user.id); reset({ newPassword: "" }); }}>
												<HiLockClosed className="h-3.5 w-3.5" aria-hidden="true" />
												Password
											</button>
											<button type="button" className={user.isActive ? "inline-flex items-center gap-1 rounded-full border border-warm/30 bg-warm-soft px-3 py-1 text-xs font-semibold text-ink transition hover:brightness-105" : "inline-flex items-center gap-1 rounded-full border border-accent/30 bg-accent-soft px-3 py-1 text-xs font-semibold text-ink transition hover:brightness-105"} onClick={() => void handleToggleStatus(user.id, user.isActive)}>
												<HiPower className="h-3.5 w-3.5" aria-hidden="true" />
												{user.isActive ? "Deactivate" : "Activate"}
											</button>
											<button type="button" className="inline-flex items-center gap-1 rounded-full border border-danger/30 bg-danger-soft px-3 py-1 text-xs font-semibold text-ink transition hover:brightness-105" onClick={() => setDeleteUserId(user.id)}>
			...
			<Modal
				...
				footer={
					<>
						<button type="button" className="rounded-2xl border border-border px-4 py-2 text-sm font-semibold text-ink" onClick={() => setPasswordUserId(null)}>
							Cancel
						</button>
						<button type="submit" form="role-password-form" className="inline-flex items-center gap-2 rounded-2xl bg-sky px-4 py-2 text-sm font-semibold text-surface" disabled={changeUserPasswordMutation.isPending}>
			...
			{banner ? <p className="rounded-2xl border border-brand/15 bg-brand-soft px-4 py-3 text-sm text-brand">{banner}</p> : null}
		</div>
	);
};
			...
			{role ? (
				<form className="grid gap-4" onSubmit={handleSubmit(onSubmit)}>
					...
					<div className="rounded-2xl border border-border bg-surface-muted px-4 py-3 text-sm text-ink-soft">
						Target role: <span className="font-semibold text-ink">{role.name}</span>
					</div>

					<div className="flex flex-wrap gap-2">
						<button type="submit" className="inline-flex items-center gap-2 rounded-2xl bg-brand px-4 py-2 text-sm font-semibold text-surface disabled:cursor-not-allowed disabled:opacity-70" disabled={createUserMutation.isPending}>
							<HiUserPlus className="h-4 w-4" aria-hidden="true" />
							{createUserMutation.isPending ? "Creating..." : `Create ${roleName}`}
						</button>
						<Link to={backTo} className="inline-flex items-center gap-2 rounded-2xl border border-border px-4 py-2 text-sm font-semibold text-ink">
							<HiCheckCircle className="h-4 w-4 text-danger" aria-hidden="true" />
							Cancel
						</Link>
					</div>
				...
				<p className="text-sm text-ink-soft">Role {roleName} not found. Create the role first.</p>
			)}
		</Panel>
	);
};
