...
const leadDisplayName = (lead: LeadIdentity) =>
...
export const LeadDetailPage = () => {
	...
	if (!leadId) {
		return (
			<Panel title="Lead">
				<div className="py-8 text-center text-gray-600">Lead not found</div>
			</Panel>
		);
	}
	...
	if (leadQuery.isLoading) {
		return (
			<Panel title="Lead">
				<div className="py-8 text-center text-gray-600">Loading lead...</div>
			</Panel>
		);
	}
	...
	const onReassignLead = async (payload: ReassignLeadFormState) => {
		...
		const validation = UpdateLeadPayloadSchema.safeParse({
			assignedTo: payload.assignedTo,
		});
		if (!validation.success) {
			const errors = validation.error.flatten().fieldErrors;
			if (errors.assignedTo?.[0]) {
				setReassignError("assignedTo", {
					type: "manual",
					message: errors.assignedTo[0],
				});
			}
			return;
		}
		...
		try {
			...
			toast.error(
				error instanceof Error ? error.message : "Unable to reassign lead",
			);
		}
	};
	...
	return (
		<div className="grid gap-6">
				...
				<div className="space-y-4">
					...
					<div className="flex justify-end gap-2">
						...
						<button
							...
							disabled={!selectedRequestCounsellor || requestDemoMutation.isPending}
			...
			<Panel
				...
				description="Lead details and workflow"
			>
				<div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
					<div className="space-y-4">
						...
						<div>
							...
							<div className="mt-2 flex flex-wrap gap-2 text-xs font-semibold uppercase tracking-[0.18em] text-gray-600">
								<span
									className={`rounded-full px-3 py-1 ${statusTone.className}`}
								>
									{statusSummary}
								</span>
								{lead?.assignedTo ? (
									<span className="rounded-full bg-sky-600/10 px-3 py-1 text-sky-600">
										Assigned
									</span>
								) : null}
								{lead?.formCompleted ? (
					...
					<div className="flex flex-wrap gap-2">
						<button
							type="button"
							className="inline-flex items-center gap-2 rounded-2xl border border-gray-300 bg-white px-4 py-2 text-sm font-semibold text-gray-900 transition-colors hover:border-blue-600 hover:text-blue-600"
						...
						<button
							type="button"
							className="inline-flex items-center gap-2 rounded-2xl bg-amber-500 px-4 py-2 text-sm font-semibold text-white hover:bg-amber-600"
						...
						{showDemoWorkflowActions ? (
							<>
								...
								<button
									type="button"
									className="inline-flex items-center gap-2 rounded-2xl bg-teal-600 px-4 py-2 text-sm font-semibold text-white"
			...
			<Panel
				...
				description="Compact view of lead information and workflow state"
			>
				<div className="grid gap-6">
					...
					<div className="grid grid-cols-1 md:grid-cols-4 gap-4">
						<div className="rounded-2xl border border-gray-200 bg-linear-to-br from-blue-50 to-blue-100/50 p-4">
							<p className="text-xs font-semibold uppercase tracking-[0.15em] text-gray-600 mb-3">
								Lead Identity
							</p>
						...
						<div className="rounded-2xl border border-gray-200 bg-linear-to-br from-emerald-50 to-emerald-100/50 p-4">
							...
							<div className="space-y-3">
								<div className="flex items-center justify-between">
									...
									<span
										className={`rounded-full px-2 py-1 text-xs font-semibold ${lead?.formSent ? "bg-emerald-200 text-emerald-700" : "bg-gray-200 text-gray-600"}`}
									>
								...
								<div className="flex items-center justify-between">
									...
									<span
										className={`rounded-full px-2 py-1 text-xs font-semibold ${lead?.formCompleted ? "bg-emerald-200 text-emerald-700" : "bg-gray-200 text-gray-600"}`}
									>
								...
								<div className="flex items-center justify-between">
									...
									<span
										className={`rounded-full px-2 py-1 text-xs font-semibold ${latestDemo ? "bg-blue-200 text-blue-700" : "bg-gray-200 text-gray-600"}`}
									>
							...
							<div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-xs">
								<div>
									...
									<p className="text-gray-900">
										{latestDemo.requestedAt
											? new Date(latestDemo.requestedAt).toLocaleDateString()
											: "-"}
									</p>
								</div>
								<div>
									...
									<p className="text-gray-900">
										{latestDemo.assignedAt
											? new Date(latestDemo.assignedAt).toLocaleDateString()
											: "-"}
									</p>
								</div>
								<div>
									...
									<p className="text-gray-900">
										{latestDemo.mentorId
											? formatUserName(
													allUsers.find(
														(user) => user.id === latestDemo.mentorId,
													)?.name ?? null,
												)
											: "-"}
									</p>
				...
				{lead?.demos?.length ? (
						...
						{lead.demos.map((demo, index) => {
							...
							return (
								<div
									...
									className={`rounded-2xl border bg-linear-to-br ${statusColor} p-5`}
								>
									<div className="flex items-start justify-between gap-3 mb-4">
										<div>
											<p className="text-sm font-bold text-gray-900">
												Demo Attempt {index + 1}
											</p>
											<p className="text-xs text-gray-600 mt-1">
												{demo.requestedAt
													? new Date(demo.requestedAt).toLocaleDateString()
													: "-"}
											</p>
										</div>
										<span
											className={`rounded-full ${statusBgColor} ${statusTextColor} px-3 py-1 text-xs font-bold`}
										>
											{demoStatus}
										</span>
									</div>
											...
											<div className="flex gap-3">
												...
												<div>
													...
													<p className="text-sm text-gray-900 font-medium">
														{new Date(demo.requestedAt).toLocaleString()}
													</p>
												</div>
											</div>
										)}
												...
												<div>
													<p className="text-xs font-semibold text-gray-600">
														...
														{demo.mentorId
															? formatUserName(
																	allUsers.find(
																		(user) => user.id === demo.mentorId,
																	)?.name ?? null,
																)
															: "Mentor"}
													</p>
													<p className="text-sm text-gray-900 font-medium">
														{new Date(demo.assignedAt).toLocaleString()}
													</p>
													{demo.demoScheduledFor && (
														<p className="text-xs text-gray-600 mt-1">
															Scheduled:{" "}
															{new Date(demo.demoScheduledFor).toLocaleString()}
														</p>
													)}
												</div>
											</div>
										...
										{demo.completedAt && (
											<div className="flex gap-3">
												...
												<div>
													...
													<p className="text-sm text-gray-900 font-medium">
														{new Date(demo.completedAt).toLocaleString()}
													</p>
												</div>
											</div>
										)}
					...
					<div className="py-12 text-center">
						<p className="text-sm text-gray-600">
							No demo attempts yet. Once a demo is requested, it will appear
							here.
						</p>
					</div>
				)}
				...
				footer={
					<>
						...
						<button
							...
							onClick={() => void handleEditSubmit(onEditLead)()}
				...
				footer={
					<>
						...
						<button
							...
							onClick={() => void handleReassignSubmit(onReassignLead)()}
							disabled={updateLeadMutation.isPending}
						>
							<HiArrowsRightLeft className="h-4 w-4" />
			...
			>
				<form
					...
					onSubmit={handleReassignSubmit(onReassignLead)}
				>
					<Controller
						name="assignedTo"
						control={reassignControl}
			...
			>
				...
				>
					<div className="rounded-2xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-700">
						A new demo request will be created. Previous mentor will be
						preselected during assignment, and you can keep or change it.
					</div>
					<Controller
						name="note"
						control={redemoControl}
			...
			<Modal
				...
				description={
					lead
						? `Default counsellor: ${defaultCounsellorId ? formatUserName(allUsers.find((user) => user.id === defaultCounsellorId)?.name ?? null) : "-"}`
						: "Select a counsellor"
				}
					...
					<Controller
						...
						render={({ field, fieldState }) => (
							<TextAreaField
								...
								value={field.value ?? ""}
								onChange={field.onChange}
								placeholder="Add admission note..."
								error={fieldState.error?.message}
							/>
						)}
					/>
			...
			>
				<div className="grid gap-4">
					<p className="text-sm text-gray-600">
						This will permanently remove the lead record from the system.
					</p>
					<TextAreaField
				...
				<div className="grid gap-4">
					<div className="flex items-start gap-3 rounded-2xl border border-amber-200 bg-amber-50 p-3">
						...
						<p className="text-sm text-amber-800">
							...
							undone. This will finalize the demo status for this lead.
						</p>
					</div>
						...
						<button
							...
							onClick={async () => {
								...
								try {
									...
									toast.success(
										"Form revoked successfully. Lead cannot access the form now.",
									);
									setRevokeConfirmOpen(false);
								} catch (error) {
									if (error instanceof ApiError) {
										toast.error(
											error.payload.message ?? "Unable to revoke form",
										);
										return;
									}
			...
			>
				<div className="flex items-start gap-3 rounded-2xl border border-red-200 bg-red-50 p-3">
					...
					<p className="text-sm text-red-800">
						<strong>Warning:</strong> Revoking this form will prevent the lead
						from accessing it. They will need a new form link to continue. This
