...
const DetailCard = ({ title, icon: Icon, children }: { title: string; icon: IconType; children: React.ReactNode }) => (
...
export const LeadEditPage = () => {
	...
	const { control, formState, handleSubmit, reset, setError } = useForm<LeadEditFormState>({
		defaultValues: {
			name: "",
			phone: "",
			level: "",
	...
	useEffect(() => {
		...
		reset({
			...
			level: lead.level ?? "",
			gender: lead.gender ?? "",
			dateOfBirth: lead.dateOfBirth ? format(new Date(lead.dateOfBirth), "yyyy-MM-dd") : "",
			...
			demoAvailability: lead.demoAvailability ? format(new Date(lead.demoAvailability), "yyyy-MM-dd'T'HH:mm") : "",
	...
	const onSubmit = handleSubmit(async (payload) => {
		...
		const updates: Record<string, unknown> = {
			...
			preferredDays: payload.preferredDays?.length ? payload.preferredDays : undefined,
		...
		if (!validation.success) {
			...
			Object.entries(errors).forEach(([field, messages]) => {
				if (messages?.[0]) {
					setError(field as keyof LeadEditFormState, { type: "manual", message: messages[0] });
				}
			});
			return;
		}
		...
		try {
			await updateMutation.mutateAsync({ leadId: lead.id, payload: validation.data });
			...
			if (error instanceof ApiError) {
				...
				Object.entries(serverErrors).forEach(([field, messages]) => {
					if (Array.isArray(messages) && messages[0]) {
						setError(field as keyof LeadEditFormState, { type: "server", message: messages[0] });
					}
				});
			...
			setBanner(error instanceof Error ? error.message : "Unable to update lead");
		}
	});
						...
						<div className="grid gap-4 md:grid-cols-2">
								...
								render={({ field, fieldState }) => (
									<SelectField
										...
										options={[
											...
											{ value: "Malayalam - English Mixed", label: "Malayalam - English Mixed" },
										]}
										error={fieldState.error?.message}
									/>
								)}
							...
							<div className="md:col-span-2">
								<Controller
									...
									render={({ field, fieldState }) => (
										<div>
											<label className="mb-1 block text-sm font-semibold text-gray-700">Student Info</label>
											<textarea
												value={field.value}
												onChange={field.onChange}
							...
							<div className="md:col-span-2">
								<label className="mb-2 block text-sm font-semibold text-gray-700">Preferred Days</label>
								<Controller
									name="preferredDays"
									control={control}
									render={({ field, fieldState }) => (
