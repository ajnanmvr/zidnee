...
export const BreakStudentsPage = () => {
	...
	const studentsQuery = useStudentsQuery(token, { scope: activeScope, status: "BREAK", search: searchTerm || undefined, sortBy, sortOrder, page, limit });
	...
	return (
		<div className="space-y-3">
			<div className="flex flex-wrap items-center gap-2 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3">
				<span className="text-sm font-semibold text-amber-800">Students on Break</span>
				<input value={searchTerm} onChange={(e) => setQueryParam("search", e.target.value)} placeholder="Search…" className="ml-auto w-52 rounded-lg border border-amber-200 px-3 py-1.5 text-sm outline-none focus:border-amber-400 bg-white" />
				<div className="flex items-center gap-1">
					<button type="button" onClick={() => setLoadAllRequested(false)} className={`rounded-lg px-3 py-1.5 text-sm font-semibold transition ${activeScope === "mine" ? "bg-amber-500 text-white" : "border border-amber-200 bg-white text-amber-700"}`}>Mine</button>
					<button type="button" onClick={() => setLoadAllRequested(true)} disabled={!canReadAll} className={`rounded-lg px-3 py-1.5 text-sm font-semibold transition ${activeScope === "all" ? "bg-amber-500 text-white" : "border border-amber-200 bg-white text-amber-700"} disabled:opacity-40`}>All</button>
				</div>
			</div>
			...
			<div className="rounded-xl border border-gray-200 bg-white overflow-hidden">
				{studentsQuery.isLoading ? (
					...
					<>
						...
						{rows.length === 0 ? <p className="py-10 text-center text-sm text-gray-400">No students currently on break.</p> : null}
