...
import { Panel, Modal, Field, TextAreaField } from "@/components/dashboard-ui";
...
export const CoursesPage = () => {
  ...
  const onSubmit = async (form: CreateCourseForm) => {
    try {
      ...
      toast.error(err instanceof Error ? err.message : "Unable to create course");
    }
  };
    ...
    <div>
      ...
      <Modal open={open} title="Create course" onClose={() => setOpen(false)}>
        <form className="grid gap-4" onSubmit={handleSubmit(onSubmit)}>
          <Controller name="name" control={control} render={({ field }) => <Field label="Course name" value={field.value} onChange={field.onChange} />} />
          <div className="grid gap-4 md:grid-cols-2">
            <Controller name="level" control={control} render={({ field }) => <Field label="Level" value={field.value} onChange={field.onChange} />} />
            <Controller name="prefix" control={control} render={({ field }) => <Field label="Prefix" value={field.value} onChange={field.onChange} />} />
          </div>
          <Controller name="description" control={control} render={({ field }) => <TextAreaField label="Description" value={field.value ?? ""} onChange={field.onChange} />} />
